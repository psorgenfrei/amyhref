class YouController < ApplicationController
  ...
  def highlights
    @hrefs = current_user.hrefs.where(good: true, good_host: true, good_path: true).where('rating > -17').order('created_at DESC, rating ASC').paginate(:page => params[:page], :per_page => 5)
